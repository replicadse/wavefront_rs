    ...
    pub fn write<W: Write>(writer: &mut W, e: &Entity) {
        match e {
            Entity::Comment{content} => {
                writer.write_all(format!("{} {}", e.token(), content).as_ref()).unwrap();
            }
            ...
            Entity::Group{name} => {
                writer.write_all(format!("{} {}", e.token(), name).as_ref()).unwrap();
            }
            Entity::Mtllib{name} => {
                writer.write_all(format!("{} {}", e.token(), name).as_ref()).unwrap();
            }
            ...
            Entity::Vertex{x, y, z, w} => {
                writer.write_all(format!("{} {} {} {}", e.token(), x, y, z).as_ref()).unwrap();
                if let Some(v) = w {
                    writer.write_all(format!(" {}", v).as_ref()).unwrap();
                }
            }
            ...
            Entity::VertexTexture{u, v, w} => {
                writer.write_all(format!("{} {}", e.token(), u).as_ref()).unwrap();
                if let Some(v) = v {
                    writer.write_all(format!(" {}", v).as_ref()).unwrap();
                    if let Some(w) = w {
