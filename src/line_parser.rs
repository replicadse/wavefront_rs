...
impl LineParser {
    fn parse_v(&self, split: &mut std::str::SplitWhitespace) -> Result<Entity, LexerError> {
        let xs = split.next();
        let ys = split.next();
        let zs = split.next();
        let ws = split.next();
        if xs == None || ys == None || zs == None {
            return Err(LexerError::new("invalid data for v"))
        }
        ...
        let w = match ws {
            Some(v) => match v.parse::<f64>() {
                Ok(v) => Some(v),
                Err(_) => return Err(LexerError::new("invalid data for v"))
            }
            None => None,
        };
    ...
    fn parse_vt(&self, split: &mut std::str::SplitWhitespace) -> Result<Entity, LexerError> {
        let xs = split.next();
        let ys = split.next();
        let zs = split.next();
        if xs == None || ys == None {
            return Err(LexerError::new("invalid data for vt"))
        }
        let x = xs.unwrap().parse::<f64>();
        let y = ys.unwrap().parse::<f64>();
        let z = match zs {
            Some(v) => match v.parse::<f64>() {
                Ok(v) => Some(v),
                Err(_) => return Err(LexerError::new("invalid data for vt"))
            }
            None => None,
        };
    ...
    fn parse_vn(&self, split: &mut std::str::SplitWhitespace) -> Result<Entity, LexerError> {
        let xs = split.next();
        let ys = split.next();
        let zs = split.next();
        if xs == None || ys == None || zs == None {
            return Err(LexerError::new("invalid data for vn"))
        }
        let x = xs.unwrap().parse::<f64>();
        let y = ys.unwrap().parse::<f64>();
        let z = zs.unwrap().parse::<f64>();
    ...
    fn parse_face(&self, split: &mut std::str::SplitWhitespace) -> Result<Entity, LexerError> {
        ...
        for vnt in split {
            ...
            if let Some(v) = vnts.next() {
                ...
                return Err(LexerError::new("could not parse face"));
            }
        }
        Ok(Entity::Face{vertices: face})
    }

    fn parse_polyline(&self, split: &mut std::str::SplitWhitespace) -> Result<Entity, LexerError> {
    ...
    pub fn parse_line(&self, split: &mut std::str::SplitWhitespace, token: &str, line: &str) -> Result<Entity, LexerError> {
        match token.to_lowercase().as_str() {
            ...
            "mtllib" => {
                if let Some(x) = split.next() {
                    ...
                    Err(LexerError::new(format!("could not parse line \"{}\"", token).as_ref()))
                }
            },
            "usemtl" => {
                if let Some(x) = split.next() {
            ...
            _ => {
                Err(LexerError::new(format!("unknown token \"{}\"", token).as_ref()))
            }
        }
    }
}