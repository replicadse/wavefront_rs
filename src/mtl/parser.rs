//! Contains logic to read entities from a `BufRead` that returns MTL formatted
//! strings.
...
impl Parser {
    ...
    fn parse_value<T: std::str::FromStr>(
        ...
        let xs = split.next();
        if xs == None {
            return Err(Box::new(crate::error::ParserError::new("invalid data")));
        }
